...
def preprocess_batch(images, bkg_color = np.array([144, 72, 17])):
    ...
    return np.rollaxis(batch_input, 1,4)
...
def play(env, policy, time=2000, preprocess=None, nrand=5):
    ...
    for _ in range(time):
        ...
        prob = policy.act(frame_input)
        
        # RIGHT = 4, LEFT = 5
        action = RIGHT if rand.random() < prob else LEFT
        frame1, re1, is_done, _ = env.step(action)
        frame2, re2, is_done, _ = env.step(0)
        
        reward.append(re1 + re2)
...
def collect_trajectories(envs, policy, tmax=200, nrand=5):
    ...
    for t in range(tmax):
        ...
        # prepare the input
        # preprocess_batch properly converts two frames
        batch_input = preprocess_batch([fr1,fr2])
        probs = policy.act(batch_input)
